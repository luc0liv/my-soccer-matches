...
SELECT 
    ...
    CAST(SUM(CASE 
        WHEN matches.home_team_goals > matches.away_team_goals THEN 3 ELSE 0 END) AS UNSIGNED) +
    CAST(SUM(CASE 
        WHEN matches.home_team_goals = matches.away_team_goals THEN 1 ELSE 0 END) AS UNSIGNED)
    AS totalPoints,
    ...
    CAST(SUM(matches.home_team_goals) - SUM(matches.away_team_goals)
